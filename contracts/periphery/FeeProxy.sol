...
contract FeeProxy is Initializable, AccessControlUpgradeable, PausableUpgradeable, IFeeProxy {
    ...
    function setBridgeGate(IBridgeGate _bridgeGate) external onlyAdmin {
        bridgeGate = _bridgeGate;
    }
    ...
    function withdrawFee(address _tokenAddress) external payable override onlyWorker whenNotPaused {
        (uint256 nativeChain, bytes memory nativeAddress) = bridgeGate.getNativeTokenInfo(
            _tokenAddress
        );
        bytes32 bridgeId = getbBridgeId(nativeChain, nativeAddress);
        ...
        bridgeGate.withdrawFee(bridgeId);
        ...
        if (chainId == nativeChain) {
            ...
            if (_tokenAddress == deEthToken) {
                ...
                _burnTransfer(bridgeId, _tokenAddress, amount, nativeChain, msg.value);
            }
            //For others tokens
            else {
                // create swap to weth
                ...
                if (chainId == ETH_CHAINID) {
                    ...
                    _burnTransfer(bridgeId, deEthToken, deEthAmount, ETH_CHAINID, msg.value);
                }
            }
        }
    ...
    function withdrawNativeFee() external payable override onlyWorker whenNotPaused {
        uint256 chainId = getChainId();
        //BridgeId of weth in ethereum network
        //TODO: can be set as contstant
        (, bytes memory nativeAddress) = bridgeGate.getNativeTokenInfo(deEthToken);
        bytes32 wethEthNetworkBridgeId = getbBridgeId(ETH_CHAINID, nativeAddress);
        ...
        // address currentTreaseryAddress = toAddress(treasuryAddresses[chainId]);
        bridgeGate.withdrawFee(getBridgeId(chainId, address(0)));
        ...
        //If we are not in Ethereum chain
            ...
            //transfer deETH to Ethereum
            _burnTransfer(wethEthNetworkBridgeId, deEthToken, deEthBalance, ETH_CHAINID, msg.value);
        }
    }
    ...
    function getbBridgeId(uint256 _chainId, bytes memory _tokenAddress)
        public
        pure
        returns (bytes32)
    {
    ...
    function _burnTransfer(
        ...
        IERC20(_erc20Token).safeApprove(address(bridgeGate), _amount);
        bridgeGate.burn{value: _nativeFixFee}(
            _bridgeId,
