...
contract DefiController is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function depositToStrategy(uint256 _amount, address _strategy) internal {
        ...
        uint256 avaliableReserves = BridgeGate.getDefiAvaliableReserves(strategy.stakeToken);
        ...
        BridgeGate.requestReserves(strategy.stakeToken, _amount);
    ...
    function withdrawFromStrategy(uint256 _amount, address _strategy) internal {
        ...
        IERC20(strategy.stakeToken).safeApprove(address(BridgeGate), 0);
        IERC20(strategy.stakeToken).safeApprove(address(BridgeGate), _amount);

        // TODO: get rewards from strategy

        // Return tokens to Gate
        BridgeGate.returnReserves(strategy.stakeToken, _amount);
