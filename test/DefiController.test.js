...
describe("DefiController", function () {
  ...
  before(async function () {
    ...
    this.BridgeFactory = await ethers.getContractFactory("MockBridgeGateForDefiController");
  ...
  it("only admin can setBridgeGate", async function () {
    await expect(
      this.defiController.connect(other).setBridgeGate(other.address)
    ).to.be.revertedWith("AdminBadRole()");
  });

  describe("with bridgeGate", function () {
    beforeEach(async function () {
      this.bridge = await this.BridgeFactory.deploy();
      await this.defiController.setBridgeGate(this.bridge.address);
    });

    it("BridgeGate is correct", async function () {
      expect(this.bridge.address).to.be.equal(await this.defiController.BridgeGate());
    });
    ...
    describe("with worker added", function () {
      ...
      describe("with strategy (inactive)", function () {
        ...
        describe("then add stakeToken and native strategies", function () {
          ...
          describe("mint stakeToken and send native eth on bridge", function () {
            beforeEach(async function () {
              await this.stakeToken.mint(this.bridge.address, tokenSupply);
              await this.bridge.sendETH({ value: nativeTokenSupply });
            });

            it("balanceOf bridge increased", async function () {
              expect(await this.stakeToken.balanceOf(this.bridge.address)).to.be.equal(
                tokenSupply
              );
              expect(await ethers.provider.getBalance(this.bridge.address)).to.be.equal(
                nativeTokenSupply
              );
            });
            ...
            describe("add bridges & connect BridgeGate", function () {
              const chainId = 1;
              const maxAmount = 0;
              ...
              beforeEach(async function () {
                ...
                await this.bridge.setDefiController(this.defiController.address);
              });
              ...
              it("rebalanceStrategy should do nothing if there is no avaliable reserves and strategy doesn't have reserves", async function () {
                ...
                // avaliableReserves == 0
                expect(await this.bridge.getDefiAvaliableReserves(token.address)).to.be.equal(0);
              ...
              it("rebalanceStrategy should do nothing if maxReservesBps equals to zero and strategy doesn't have reserves", async function () {
                ...
                // add bridge with non zero balance
                await this.bridge.addBridge(
                  token.address,
                  chainId,
                  maxAmount,
                ...
                // avaliableReserves > 0
                expect(await this.bridge.getDefiAvaliableReserves(token.address)).to.be.not.equal(
                  0
                );
              ...
              describe("After strategies rebalanced", async function () {
                ...
                beforeEach(async function () {
                  stakeTokenAvaliableReserves = await this.bridge.getDefiAvaliableReserves(
                    this.stakeToken.address
                  );
                ...
                it("rebalanceStrategy should withdraw all if avaliableReserves equals to zero and strategy have reserves", async function () {
                  // set bridge's _minReservesBps to 100%
                  const bridgeId = await this.bridge.getBridgeId(
                    chainId,
                    this.stakeToken.address
                  );
                  await this.bridge.updateAsset(bridgeId, 0, this.BPS_DENOMINATOR, 0);
